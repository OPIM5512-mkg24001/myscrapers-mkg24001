# main.py (VERSION 2)
# Build a single, ever-growing CSV from all structured JSONL files that match the new schema.
...
# V2 UPDATE: Added 'transmission' and 'fuel_type' to the schema
CSV_COLUMNS = [
...
def _jsonl_records_for_run(bucket: str, structured_prefix: str, run_id: str):
    ...
    for blob in b.list_blobs(prefix=prefix):
        ...
        try:
            ...
            # V2 HACK: Only yield records that contain our new fields!
            # If they don't have these keys, it skips them to protect the schema.
            if "transmission" not in rec or "fuel_type" not in rec:
                continue
...
def materialize_http(request: Request):
    ...
    try:
        ...
        # V2 UPDATE: Save as _v2.csv so we don't overwrite the original pipeline!
